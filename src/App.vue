    ...
    <div v-if="hasInjected==true">
      ...
      >
        ...
        <v-toolbar-title class="pr-md-12">
          <v-img src="./assets/light.png" max-width="90" max-height="60" class="d-none d-md-block" v-if="!$vuetify.theme.dark" contain />
          <v-img src="./assets/dark.png" max-width="90" max-height="60" class="d-none d-md-block" v-if="$vuetify.theme.dark" contain />
          <v-img src="./assets/light.png" max-width="90" max-height="60" class="d-block d-md-none" v-if="!$vuetify.theme.dark" contain />
          <v-img src="./assets/dark.png" max-width="90" max-height="60" class="d-block d-md-none" v-if="$vuetify.theme.dark" contain />
        </v-toolbar-title>
        <v-btn
          to="/staking"
        ...
        >
         ...
         <v-btn
         v-if="$store.state.currentAddress==$store.state.owner"
          to="/settings"
          class="d-none d-md-flex text-caption text-capitalize ml-1"
          color="#00ACC1"
          text
        >
          <ToolIcon size="1.5x" class="custom-class mr-1"></ToolIcon>Owner settings
        </v-btn>
        <v-spacer></v-spacer>
        <!-- <v-btn class="ma-2" small color="#00ACC1">Connect Wallet</v-btn> -->
        <v-btn
          ...
          <v-card>
            <v-list class="pr-5">
              <v-list-item>
                ...
                <v-list-item-action>
                  <v-switch color="#00ACC1" v-model="$vuetify.theme.dark"></v-switch>
                </v-list-item-action>
              </v-list-item>
            </v-list>
          </v-card>
...
import particlesJS from "./views/particelsJS"
import { ZapIcon, SettingsIcon, MoonIcon,ToolIcon } from "vue-feather-icons";
export default {
  name: "App",
  components: {
    ZapIcon,
  ...
  data: () => ({
    ...
    items: [
      ...
      {title:"Settings",to:"/settings",component:'ToolIcon'}
    ],
    primaryDrawer: {
      model: null,
    },
  ...
  computed: {
    color(){
      return this.$vuetify.theme.dark ? '#00ACC1' : '#00ACC1'
    }
  },
  async created() {
      ...
      if (
        ...
        if (network == "main") {
          this.hasInjected = true;
           //window.web3 = new Web3('https://mainnet.infura.io/v3/53ca560c81ca4899b53357f6597b6909')
