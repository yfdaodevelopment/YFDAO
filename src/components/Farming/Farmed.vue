<template>
  <v-card class="mx-auto" height="270" outlined>
    ...
    <v-card-text class="font-weight-light my-2 px-7" style="font-size: 18px; height: 120px">
      {{this.$store.state.totalFarmedMe}} YFDAO <span @click="updateReward" style="position:relative;bottom:2px;cursor:pointer"><v-icon small>mdi-refresh</v-icon></span>
    </v-card-text>
    <hr class="hr-styles" />
    <v-card-actions  style="padding:16px;">
      <v-btn :disabled="$store.state.totalFarmedMe>0?false:true" @click="claimFarmReward" class="ml-2" color="#00ACC1" style="font-size: 12px; width: 150px;color:white" v-bind="attrs" v-on="on" elevation="0">
...
import utils from "../../utils/abi";
    ...
    methods: {
       async claimFarmReward(){
        try{
          ...
          if(balance<ethfee){
            this.$store.state.text=`You don't have enough ether to pay claiming fee of ${ethfee} ether. Your current ETH balance is ${balance.toFixed(4)} ETH`
  ...
  async created() {
    try{
      console.log("items:",this.sortedItems[0]);
