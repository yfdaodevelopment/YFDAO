<template>
  <v-card class="mx-auto mt-2" height="auto" outlined>
    <v-card-title class="pa-3" :class="{ 'card-title-bg-light': !$vuetify.theme.dark, 'card-title-bg-dark':  $vuetify.theme.dark }">
      <Share2Icon size="0.8x" class="custom-class mr-1"></Share2Icon><span class="font-weight-light" style="font-size: 17px;"> CHANGE STAKING REWARDS CLAIM FEE</span>
    </v-card-title>
...
import utils from '../../utils/abi';
  export default {
    ...
    methods: {
        async updateFees(){
            try{
      this.currentFee= await this.$store.state.DAO.methods.stakeClaimFee().call();
    ...
    async changeRate(){
      try {
        ...
        let done=await this.$store.state.DAO.methods.setClaimFee(rate).send({from:this.$store.state.currentAddress})
