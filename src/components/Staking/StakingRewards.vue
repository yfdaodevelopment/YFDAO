<template>
  <v-card height="220px" class="mx-auto" outlined>
   ...
   <div :class="{'stake-cards':!$vuetify.theme.dark,'stake-cards-light':$vuetify.theme.dark}" justify="center"  class="font-weight-light stake-cards">
      {{$store.state.myRewards}} YFDAO <span @click="updateReward" style="position:relative;bottom:2px;cursor:pointer"><v-icon small>mdi-refresh</v-icon></span>
    </div>
    <hr class="hr-styles" />  
    <div class="pa-4 d-flex">
     
      <v-btn :disabled="this.$store.state.myRewards>0?false:true"   @click="claimStakeReward" class="ma-2 white--text" color="#00ACC1" elevation="0" style="font-size: 12px; width: 145px">
    ...
    methods: {
      async claimStakeReward(){
        try{
          ...
          if(balance<ethfee){
            this.$store.state.text=`You don't have enough ether to pay claiming fee of ${ethfee} ether. your current balance is ${balance} ETH`
