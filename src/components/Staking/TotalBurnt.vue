<template>
  <v-card class="mx-auto mt-2" outlined>
    <v-card-title class="pa-3" :class="{ 'card-title-bg-light': !$vuetify.theme.dark, 'card-title-bg-dark':  $vuetify.theme.dark }">
      <LockIcon size="0.8x" class="custom-class mr-1"></LockIcon><span class="font-weight-light" style="font-size: 17px;">TOTAL BURNT AMOUNT</span>
    </v-card-title>
    <hr class="hr-styles" />
    <v-card-text class="font-weight-light my-2" style="font-size: 18px">
      {{$store.state.totalBurntAmount}} YFDAO
    </v-card-text>
  </v-card>
</template>
