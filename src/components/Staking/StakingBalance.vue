<template>
  <v-card class="mx-auto" height="220px" outlined>
    ...
    <hr class="hr-styles" />
    <div :class="{'stake-cards':!$vuetify.theme.dark,'stake-cards-light':$vuetify.theme.dark}" justify="center"  class="font-weight-light stake-cards">{{$store.state.breedBalance}} YFDAO</div>
    <hr class="hr-styles" />
      ...
      if (this.amount == 0) {
       ...
       this.$store.state.color="red"
        this.$store.state.text =
          "You don't have enough YFDAO tokens to stake the specified amount.";
          this.$store.state.snackbar = true;
        return;
      } else if (this.balance >= this.amount) {
        try {
          ...
          let done = await this.$store.state.BREED.methods
            .increaseAllowance(utils.daoAddress, amount)
